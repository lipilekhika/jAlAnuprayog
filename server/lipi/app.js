...
if (true) { //redirects
    let rdr = {
        "favicon.ico": "https://cdn.jsdelivr.net/gh/lipilekhika/dist@latest/img/main.ico",
...
if (true) { //convert
    var convert = async (arg, bd) => {
        ...
        else if (arg.length == 2 && "text" in bd)
            u = [arg[0], arg[1], bd.text];
        else if (arg.length == 3)
            u = [arg[0], arg[1], arg[2]];
        if (u.length != 3)
            return ["ERROR in request format\nRefer to Lipi Lekhika API Documentaion", 404];
        else {
            ...
            if (lipi.k.in(lipi.k.langs, u[0]) && lipi.k.in(lipi.k.langs, u[1])) {
                ...
                return ["Unsupported Language or Wrong Language Code", 404];
        }
    };
